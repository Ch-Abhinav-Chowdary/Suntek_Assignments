...
const TaskItem = ({title,index,priority,completed,updateStatus}) => {
  return (
    <div className='bg-indigo-400 w-76 rounded-2xl p-5 shadow-2xl shadow-gray-400 '>
        <h1 className='text-2xl font-bold'>{title}</h1>
        <p className='text-lg font-semibold'>priority: {priority}</p>
        
        <span className='flex gap-3'>Status: 
            {
            completed ? <p className='font-semibold'>Completed</p> : <p className='font-semibold' >In Progress</p>
            }
        </span>
        {
